...
(function($)
{
	...
	function buildTable(obj,options) {
		...
		if(hasTable && hasTHead && hasTBody) {
			...
			var table = obj.find('div.fht_table_body table').first();
            ...
            if (obj.find('div.fht_fixed_header').size() == 0) {
                obj.prepend('<div class="fht_fixed_header"><table cellspacing="0" width="'+obj.find('div.fht_table_body table').width()+'px"></table></div>');
            }
            obj.find('div.fht_fixed_header').css({'width':parentDivWidth + 'px', 'height':table.find('thead').outerHeight() + 'px'});
            
            var i = 0;
            obj.find('div.fht_table_body table thead tr:first > th').each(function() {
                var width = ($.browser.msie == 'true') ? $(this).width() + 10 : $(this).width() -1 ;
